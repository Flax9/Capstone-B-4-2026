...
func ConnectDatabase() {
	...
	if err != nil {
		log.Fatal("[transaction-worker] Gagal menyambung ke Master via PgBouncer: ", err)
	}

	sqlDB, err := DB.DB()
	if err != nil {
		log.Fatal("[transaction-worker] Gagal mendapatkan SQL DB: ", err)
	}
	// Worker hanya butuh koneksi yang sedikit tapi stabil
	...
	}).SetMaxIdleConns(3).SetMaxOpenConns(10).SetConnMaxLifetime(5 * time.Minute))

	if err != nil {
		log.Fatal("[transaction-worker] Gagal memasang DBResolver: ", err)
	}
	log.Println("[transaction-worker] Terhubung ke PostgreSQL via PgBouncer!")
}
...
func ConnectRedis() {
	...
	if err := RedisClient.Ping(Ctx).Err(); err != nil {
		log.Fatal("[transaction-worker] Gagal menyambung ke Redis: ", err)
	}
	log.Println("[transaction-worker] Terhubung ke Redis!")
}
