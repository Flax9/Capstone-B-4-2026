...
func ConnectDatabase() {
	...
	if err != nil {
		log.Fatal("[balance-service] Gagal menyambung ke Master via PgBouncer: ", err)
	}

	sqlDB, err := DB.DB()
	if err != nil {
		log.Fatal("[balance-service] Gagal mendapatkan SQL DB: ", err)
	}
	...
	if err != nil {
		log.Fatal("[balance-service] Gagal memasang DBResolver: ", err)
	}
	log.Println("[balance-service] Terhubung ke PostgreSQL via PgBouncer!")
}
...
func ConnectRedis() {
	...
	if err := RedisClient.Ping(Ctx).Err(); err != nil {
		log.Fatal("[balance-service] Gagal menyambung ke Redis: ", err)
	}
	log.Println("[balance-service] Terhubung ke Redis!")
}
